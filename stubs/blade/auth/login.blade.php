...
@section('content')
    <div class="flex flex-col break-words bg-gray-900 sm:rounded-lg sm:shadow-lg sm:p-10">
        ...
        <form class="w-full p-6" method="POST" action="{{ route('login') }}">
            ...
            <div class="flex flex-col flex-wrap mb-6">
                <input autocomplete="current-password" id="password" placeholder="Password" type="password" class="input bg-gray-800 rounded text-gray-200 input-minimal border-gray-700 w-full {{ $errors->has('password') ? ' border-red-500' : '' }}" name="password" required>
